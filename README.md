...
> **Status: work in progress.** The method is under active validation
> and has not yet been checked quantitatively against an independent
> ground truth (GOES-East ABI is the natural reference; a collocation
> pipeline is in development). Algorithm parameters, thresholds, and
> even the radiometric-confirmation surrogate may change as validation
> progresses. Known failure modes documented so far include false
> positives over open ocean, false negatives over snow and other
> bright surfaces, and shadow-length underestimation for deep
> convection. When citing results in downstream work, please pin the
> commit hash you ran.
