...
| `paper_draft.md` | Paper draft in Markdown. **Stale snapshot** — the live version is on Overleaf (see § 0). |
| `paper/` | LaTeX snapshot of the draft: `paper.tex`, `references.bib`, `Makefile`, `README.md`, `figures/`. **Stale snapshot** — the live version is on Overleaf (see § 0). |
